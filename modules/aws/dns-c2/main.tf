...
resource "aws_key_pair" "dns-c2" {
  key_name = "dns-c2-key-${random_id.server.hex}"
  public_key = tls_private_key.ssh.public_key_openssh
}

resource "aws_instance" "dns-c2" {
  tags = {
    Name = "dns-c2-${random_id.server.hex}"
  }

  ami = var.amis[data.aws_region.current.name]
  instance_type = var.instance_type
  key_name = aws_key_pair.dns-c2.key_name
  vpc_security_group_ids = concat(var.security_groups, [aws_security_group.dns-c2.id])
...
data "template_file" "ssh_config" {
  ...
  vars = {
    name = "dns_c2_${aws_instance.dns-c2.private_ip}"
    hostname = aws_instance.dns-c2.private_ip
    user = var.user
    identityfile = "${local.ssh-keys-path}/${aws_instance.dns-c2.private_ip}"
  }
}
...
resource "null_resource" "gen_ssh_config" {
  ...
  triggers = {
    ...
    serverIp = aws_instance.dns-c2.private_ip
  }
